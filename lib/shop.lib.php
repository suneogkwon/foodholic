    ...
    for($i=0; $i<$subj_count; $i++) {
        ...
        if($opt_count) {
            ...
            if($is_div === 'div') {
                ...
                $str .= '<label for="it_supply_'.$seq.'" class="label-title padding-bottom-1">'.$subj[$i].'</label>'.PHP_EOL;
            } else {
                $str .= '<tr>'.PHP_EOL;
...
function print_item_options($it_id, $cart_id)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        if($i == 0)
            $str .= '<ul class="no-bullet margin-0">'.PHP_EOL;
        $price_plus = '';
        if($row['io_price'] >= 0)
            $price_plus = '+';
        $str .= '<li class="position-relative margin-bottom-1" style="padding-left: 40px;">'.get_text($row['ct_option']).' '.$row['ct_qty'].'개 ('.$price_plus.display_price($row['io_price']).')</li>'.PHP_EOL;
    }
