...
// 신규회원 쿠폰발생
if(false && $w == '' && $default['de_member_reg_coupon_use'] && $default['de_member_reg_coupon_term'] > 0 && $default['de_member_reg_coupon_price'] > 0) {
    $j = 0;
    $create_coupon = false;
...
if($w == ''){
    $sql = " select * from {$g5['g5_shop_coupon_zone_table']} where cz_type = '2' ";
    $res = sql_query($sql);
    ...
    for($k=0; $cp=sql_fetch_array($res); $k++) {
        ...
        VALUES
            ( '$cp_id', '{$cp['cz_subject']}', '{$cp['cp_method']}', '{$cp['cp_target']}', '$mb_id', '{$cp['cz_id']}', '$cp_start', '$cp_end', '{$cp['cp_type']}', '{$cp['cp_price']}', '{$cp['cp_trunc']}', '{$cp['cp_minimum']}', '{$cp['cp_maximum']}', '".G5_TIME_YMDHIS."' ) ";
