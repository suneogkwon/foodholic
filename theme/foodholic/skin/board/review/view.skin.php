...
$ca_arr = array();
$sql = " select bo_category_list from g5_board where bo_table = 'review'";
$ca = sql_fetch($sql);
$ca_arr = explode('|',$ca['bo_category_list']);
?>
    ...
    <header>
        <h2 id="bo_v_title">
            ...
            <span class="bo_v_cate"><?php // echo $view['ca_name']; // 분류 출력 끝 
                $ca_name = explode(',',$view['ca_name']);
                for($k=0;$k<count($ca_name);$k++){
                    for($l=0;$l<count($ca_arr);$l++){
                        if($ca_arr[$l] == $ca_name[$k])
                            echo '<span class="bo_cate_link nth_'.$l.'">'.$ca_arr[$l].'</span>';
                    }
                }
            ?></span> 
            <?php } ?>
    ...
    <section id="bo_v_info">
        ...
        <div class="profile_info">
        	...
        	<div class="profile_info_ct">
                <span class="sound_only">작성자</span> <strong><?php // echo $view['name'] 
                    if($is_admin){
                        echo '<a href="'.G5_ADMIN_URL.'/point_list.php?sfl=mb_id&stx='.$list[$i]['mb_id'].'">'.$view['name'] .'</a>';
                    } else{
                        echo $view['name'] ;
                    }
                ?><?php if ($is_ip_view && false) { echo "&nbsp;($ip)"; } ?></strong><br>
...
;
    ...
    <section id="bo_v_atc">
        ...
        <div id="bo_v_share">
	        ...
	        <?php if ($scrap_href && false) { ?><a href="<?php echo $scrap_href;  ?>" target="_blank" class="btn btn_b03" onclick="win_scrap(this.href); return false;"><i class="fa fa-bookmark" aria-hidden="true"></i> 스크랩</a><?php } ?>
	    </div>
