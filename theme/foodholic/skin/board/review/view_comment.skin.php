        ...
        <div class="cm_wrap">
            ...
            <div class="cmt_contents">
                ...
                if ($file[0]['view']) { //이미지 파일이면
                    ...
                    echo "</div>\n";
                } else if (isset($file[0]['source']) && $file[0]['source'] && !$file[0]['view']) { //이미지 파일이 아니면 다운로드 출력
                    echo "<div id=\"cmt_vc_file\">\n";
                    $ss_name = 'ss_view_'.$bo_table.'_'.$list[$i]['wr_id'];
                    if (!get_session($ss_name)) set_session($ss_name, TRUE);
                    echo "<a href=".$file[0]['href']." class='view_file_download'><i class='fa fa-download'></i>".$file[0]['source']."</a>";
                    echo "</div>\n";
                }
                ?>
                <!-- } 댓글 첨부파일 출력 끝 -->
    ...
    <div class="bo_vc_w_wr">
        <div class="bo_vc_w_info">
            ...
            <div id="cmt_file_wrap">
                ...
                <div id="cmt_file_del"><input type="checkbox" name="bf_file_del" value="1"> <label>파일삭제 (<span id="cmt_file_source"></span>)</label></div>
            </div>
            <?php } ?>
            <!-- } 댓글 첨부파일 끝 -->
            <?php if ($is_guest) { ?>
    ...
    if (save_before != el_id)
    {
        ...
        if (work == 'cu')
        {
            ...
            if (document.getElementById('wr_file_comment_'+comment_id).value > 0) {
                document.getElementById('cmt_file_del').style.display = 'inline-block';
                document.getElementById('cmt_file_source').innerHTML = document.getElementById('wr_filesource_comment_'+comment_id).value;
            } else {
                document.getElementById("cmt_file_del").style.display = 'none';
            }
            // 댓글 첨부파일 끝
