...
?>
    <ul class="sod_list">
        ...
        $sql2 = " select count(CASE WHEN b.ca_id = '10' THEN '1' END) as cnt, count(CASE WHEN b.ca_id = '20' THEN '1' END) as cnt2
                   from {$g5['g5_shop_cart_table']} a left join {$g5['g5_shop_item_table']} b on ( a.it_id = b.it_id )
                  where a.od_id = '$s_cart_id'
                    and a.ct_select = '1' ";
        $result2 = sql_fetch($sql2);

        $lunch_cnt = $result2['cnt'];
        $side_cnt = $result2['cnt2'];
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
            ...
            if($row['ca_id'] == 10 || $row['ca_id'] == 20)
                $chk_category = true;

            if($row['ca_id'] == 10 && ($lunch_cnt >= $minimum_day[0]) && $row['ct_binding'] == 0){
                $sql = " update {$g5['g5_shop_cart_table']} set ct_point = ct_point + '{$day_point[0]}', ct_binding = 1
            ...
            if($row['ca_id'] == 20 && ($side_cnt >= $minimum_day[1]) && $row['ct_binding'] == 0){
                $sql = " update {$g5['g5_shop_cart_table']} set ct_point = ct_point + '{$day_point[1]}', ct_binding = 1
                        where od_id = '$s_cart_id'
                        and it_id  = {$row['it_id']}
                        and ct_select = '1' ";
                sql_fetch($sql);
            ...
            if(mb_substr($row['it_id'],6,2) == 10){
                $image = get_it_image(1, $image_width, $image_height);
            } else if (mb_substr($row['it_id'],6,2) == 20){
                $image = get_it_image2(1, $image_width, $image_height);
            } else {
                $image = get_it_image($row['it_id'], $image_width, $image_height);
            }
            ...
            if($it_options) {
                $mod_options = '<div class="sod_option_btn"><button type="button" class="mod_options">옵션수정</button></div>';
                $ot_name = '<div class="sod_opt">'.$it_options.'</div>';
            }
        ...
        <li class="sod_li">
            ...
            <div class="li_op_wr">
                ...
                <?php if($row['ca_id'] == 10 || $row['ca_id'] == 20){ ?> 
                    <div class="sod_explan"><?php echo stripslashes($row['it_explan']) ?></div>
                <?php } ?>
...
<div id="sod_frm" class="sod_frm_mobile">
    <form name="forderform" class="2017_renewal_itemform" method="post" action="<?php echo $order_action_url; ?>" autocomplete="off">
            ...
            <ul>
                ...
                <li>
                    <label for="od_pwd">비밀번호<strong class="sound_only"> 필수</strong></label>
                        <input type="password" name="od_pwd" id="od_pwd" required class="frm_input required" maxlength="20" placeholder="영,숫자 3~20자 (주문서 조회시 필요)">
                        
                </li>
                <?php } ?>

                <li>
                    <label for="od_tel">전화번호</label>
                    <input type="text" name="od_tel" value="<?php echo get_text($member['mb_tel']); ?>" id="od_tel" class="frm_input" maxlength="20">
                </li>
                <li>
                    <label for="od_hp">핸드폰<strong class="sound_only"> 필수</strong></label>
                ...
                <li>
                    ...
                    <button type="button" class="btn_frmline btn_addsch" onclick="win_zip('forderform', 'od_zip', 'od_addr1', 'od_addr2', 'od_addr_jibeon');">주소검색</button></span>
                ...
                <?php if($chk_category) {?>
                <li>
                    <label for="od_addr3">수령방법<strong class="sound_only"> 필수</strong></label><br>
                    <label><input type="radio" name="od_addr3" value="방문수령" onclick="hideDeliveryForm();">방문수령(<?php echo $goods_cnt * $hall_discount?>원 할인)</input></label><br>
                    <label><input type="radio" name="od_addr3" value="배달수령" onclick="showDeliveryForm();" checked>배달수령</input></label>
                </li>
                <?php } ?>
        ...
        <div class="odf_list">
            <ul>
                ...
                <li>
                    ...
                    <button type="button" class="btn_frmline  btn_addsch" onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr_jibeon');">주소 검색</button></span>
                    ...
                    <label><input type="radio" name="od_b_addr3" value="공동현관 비번없음" onclick="hidetextform();" checked>공동현관 비번없음</input></label>
                    <label><input type="radio" name="od_b_addr3" value="경비실호출" onclick="hidetextform();" checked>경비실호출</input></label>
                    <label><input type="radio" name="od_b_addr3" value="자유출입가능" onclick="hidetextform();" checked>자유출입가능</input></label>
                    <label><input type="radio" name="od_b_addr3" value="" onclick="showtextform();" checked>공동현관 있음</input></label>
                    <input style="width: 200px;" type="text" name="od_b_addr3_text" id="od_b_addr3_text" class="frm_input frm_address required" required size="60" value="" placeholder="공동현관 비밀번호">
...
<script>
var zipcode = "";
var chk_recieve_way = false;
var hall_discount = <?php echo $goods_cnt * $hall_discount ?>;
...
$(function() {
    ...
    $(".mod_options").click(function() {
        ...
        $.post(
            g5_theme_shop_url + "/orderoption.php", {
                it_id: it_id
            },
            function(data) {
...
function showtextform(){
    $('#od_b_addr3_text').show();
}

function hidetextform(){
    $('#od_b_addr3_text').hide();
}

function showDeliveryForm(){
    if(chk_recieve_way){
        send_cost = parseInt($('input[name=od_price]').val()) - hall_discount;
        $('#sod_frm_taker').show();
        $('input[name=od_price]').val(send_cost);
        $('input[name=good_mny]').val(send_cost);
        reload_tot()
    }
    chk_recieve_way = false;
}
...
function hideDeliveryForm(){
    ...
    chk_recieve_way = true;
}

function reload_tot(){
    var fee = parseInt($('.sod_bsk_sell strong').text().replace(',',''));
    var tot = parseInt($('#ct_tot_price').text().replace(',',''));
    var price = parseInt($('#od_tot_price').text().replace(',',''));

    hall_discount = -1 * hall_discount;
        
    $('.sod_bsk_sell strong').text(number_format(fee + hall_discount));
    $('#ct_tot_price').text(number_format(tot + hall_discount));
    $('#od_tot_price').text(number_format(price + hall_discount));
...
function orderfield_check(f)
{
    ...
    check_field(f.od_hp, "주문하시는 분 휴대폰 번호를 입력하십시오.");
    ...
    if(f.od_addr3[1].checked == true){
        check_field(f.od_b_name, "받으시는 분 이름을 입력하십시오.");
        check_field(f.od_b_hp, "받으시는 분 휴대폰 번호를 입력하십시오.");
        check_field(f.od_b_addr1, "주소검색을 이용하여 받으시는 분 주소를 입력하십시오.");
        //check_field(f.od_b_addr2, "받으시는 분의 상세주소를 입력하십시오.");
        if(f.od_b_addr3[3].checked == true){
            if(f.od_b_addr3_text.value == "") {
                check_field(f.od_b_addr3_text, "공동현관 비밀번호를 입력하십시오.")
            } else {
                f.od_b_addr3[3].value = f.od_b_addr3_text.value;
            }
