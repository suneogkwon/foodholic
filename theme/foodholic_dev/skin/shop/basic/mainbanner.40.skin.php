...
for ($i = 0; $row = sql_fetch_array($result); $i++) {
    ...
    if ($i == 0) echo '<div id="survey_bn"><div class="floatwrap">' . PHP_EOL;
    //print_r2($row);
    // 테두리 있는지
    ...
    if (file_exists($bimg)) {
        ...
        $item_html .= '<div class="item nth_'.$i.'">';
        
        $item_html .= '<img src="' . G5_DATA_URL . '/banner/' . $row['bn_id'] . '" width="' . $size[0] . '" alt="' . get_text($row['bn_alt']) . '"' . $bn_border . '>';
...
if ($i > 0) {
    // echo '<div id="top_bx_pager" class="bx_pager">
    // <ul>';
    // $k = 0;
